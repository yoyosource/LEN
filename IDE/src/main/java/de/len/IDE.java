package de.len;

import yapion.annotations.registration.YAPIONAccessGenerator;
import yapion.hierarchy.types.YAPIONValue;
import yapion.parser.YAPIONParser;

import java.util.ArrayList;
import java.util.List;

public class IDE {

    @YAPIONAccessGenerator(lombokToString = true)
    private static final String flavour = "src/main/resources/flavour.yapion";

    private static List<Flavour> flavourList = new ArrayList<>();

    public static void main(String[] args) {
        YAPIONParser.parse(IDE.class.getResourceAsStream("/flavours.yapion")).getArray("").forEach(yapionAnyType -> {
            String flavour = ((YAPIONValue<String>) yapionAnyType).get();
            flavourList.add(new Flavour(YAPIONParser.parse(IDE.class.getResourceAsStream("/flavours/" + flavour + ".flavour"))));
        });
        flavourList.forEach(System.out::println);
    }
}
